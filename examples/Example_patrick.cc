...
void Solve(Action &D,LatticePropagator &source,LatticePropagator &propagator)
{
  ...
  // Red black solver above (SchurRedBlackDiagMooeeSolve<LatticeFermion>) requires an OperatorFunction to be passed in, 
  // which in this case is a CG solver. The OperatorFunction is called "HermitianRBSolver", so seems like you need a 
  // Hermitian red-black solver to be passed into the constructor for SchurRedBlackDiagMooeeSolve.
...
int main (int argc, char ** argv)
{
  ...
  // GridCartesian         * UGrid   = SpaceTimeGrid::makeFourDimGrid(GridDefaultLatt(), 
	...
	// 							   GridDefaultMpi());
  // think here I want to change GridDefaultLatt() to the proper coordinates
  ...
  std::cout << "Running this example." << std::endl;
  ...
  std::vector<RealD> masses({ 0.03 } ); // u/d, s, c ??
