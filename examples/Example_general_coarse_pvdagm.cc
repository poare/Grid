 ...
 */
void poly_coeffs(std::vector<std::complex<double>> &coeffs, LinearOperatorBase<LatticeFermion> &DiracOp, LatticeFermion src, LatticeFermion psiStar, GridCartesian* FGrid, int N)
{
  // kryBasis = {b, Db, D^2 b, ..., D^N b}, kryBasis = {k0, k1, ..., kN}
  ...
  // orthonormalize canonical Krylov basis {b, Db, D^2 b, ..., D^N b} <--> {k_i} and compute components <k_i | psi*>
...
//   Coordinate clatt = GridDefaultLatt();              // [PO] initial line before I edited it
  ...
  // block 2.2.2.2 in 4d, don't block in 5d to create near-null vectors
...
//   ShiftedPVdagM_t ShiftedPVdagM(0.01,Ddwf,Dpv); // 
  ...
  */
  ...
  // constructs the little Dirac op < phi_j^a | D | phi_k^b >
  /*
