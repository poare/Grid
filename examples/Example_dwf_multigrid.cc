...
// Copied from Test_dwf_multigrid.cc
...
int main (int argc, char ** argv)
{
  ...
  std::cout<<GridLogMessage << "GRID INITED" << std::endl;
...
//   std::string file("./ckpoint_lat.4000");
  ...
  std::cout << GridLogMessage << "Acting Dpv on a vector of ones (TODO: import into Petsc)" << std::endl;

  // CoarseVector c_src (Coarse5d);
  // Complex one(1.0);
  // c_src = one;
  LatticeFermion ones (FGrid);
  ones = 1.0;
  std::cout << GridLogMessage << "norm2(1 vector) = " << norm2(ones) << std::endl;

  LatticeFermion Dpv1 (FGrid);
  Dpv.M(ones, Dpv1);
  std::cout << GridLogMessage << "norm2(Dpv(1 vector)) = " << norm2(Dpv1) << std::endl;
...
//   ConjugateGradient<LatticeFermion>          FineCG(tol,MaxIt);
  ...
  // currently testing PVdagM with BiCGstab so comment all others out
...
//   PVdagMLinearOperator<Level1Op,CoarseVector> CoarsePVdagM(LDOp,LDOpPV);
  ...
  /* Added this comment to just run Bicgstab
